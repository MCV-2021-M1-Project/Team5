...
def backgroundFill(queryImage):
    ...
    #Draw contours
    contours = cv2.Canny(queryImageGray,30,150)
    ...
    contours[0:40, :], contours[-40:, :], contours[:, 0:40], contours[:, -40:] = 0, 0, 0, 0
    ...
    # contoursDetect, hierarchy = cv2.findContours(queryImageGray,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    # cv2.drawContours(queryImageGray, contoursDetect, -1, (0, 255, 0), 30)
    
    # cv2.fillPoly(contours, pts = contoursDetect, color=255)
...
def backgroundRemoval(maskFill, filename):
    ...
    kernelSize = (round(maskFill.shape[0]/16), round(maskFill.shape[1]/40))
